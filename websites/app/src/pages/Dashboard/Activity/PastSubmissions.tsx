...
const QUERY = `
  query PastItems($userAddress: Bytes!, $first: Int!, $skip: Int!) {
    litems(
      where: {
        status_in: [Registered, Absent]
...
const PastSubmissions: React.FC<Props> = ({ totalItems, address }) => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["pastItems", queryAddress, skip],
  ...
  if (!data || data.length === 0) return <>No past submissions.</>;
  return (
    <>
      {data.map((item) => (
