...
const FilterModal: React.FC<FilterModalProps> = ({ 
  ...
  const registrationStatuses = useMemo(() => {
    ...
    // If no status filters in URL, default to all statuses (show all by default)
    return statuses.length > 0 ? statuses : REGISTRATION_STATUSES;
  }, [searchParams]);
  
  const disputedValues = useMemo(() => {
    const disputed = searchParams.getAll('disputed');
    // If no disputed filters in URL, default to all values (show all by default)
    return disputed.length > 0 ? disputed : ['true', 'false'];
  }, [searchParams]);
  ...
  const handleStatusChange = useCallback((status: string) => {
    setSearchParams((prev) => {
      ...
      // Always maintain userAddress filter
      if (userAddress) {
