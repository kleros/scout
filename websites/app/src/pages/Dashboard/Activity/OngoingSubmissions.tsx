...
const OngoingSubmissions: React.FC<Props> = ({ totalItems, address, chainFilters = [] }) => {
  ...
  const status = statusParams.length > 0 ? statusParams.filter(s => defaultOngoingStatuses.includes(s)) : defaultOngoingStatuses;
  const disputed = disputedParams.length > 0 ? disputedParams.map(d => d === 'true') : [true, false];
  
  const searchTerm = searchParams.get('search') || '';
  
  const { data, isLoading } = useQuery({
    queryKey: ["ongoingItems", queryAddress, currentPage, status, disputed, orderDirection, chainFilters, searchTerm],
  ...
  if (!data || data.items.length === 0) return <>No ongoing submissions.</>;
